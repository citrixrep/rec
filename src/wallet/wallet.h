...
static const unsigned int MAX_FREE_TRANSACTION_RECTE_SIZE = 1000;
...
public:
    ...
    void UpdateTimeFirstKey(int64_t nRECteTime);
    ...
    bool AddWatchOnly(const CScript& dest, int64_t nRECteTime);
     ...
     * calling RECteTransaction();
     */
    bool FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, bool overrideEstimatedFeeRate, const CFeeRate& specificFeeRate, int& nChangePosInOut, std::string& strFailReason, bool includeWatching, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, bool keepReserveKey = true, const CTxDestination& destChange = CNoDestination());

    /**
     * RECte a new transaction paying the recipients with a set of coins
     * selected by SelectCoins(); Also create the change output, when needed
     * @note passing nChangePosInOut as -1 will result in setting a random position
     */
    bool RECteTransaction(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosInOut,
    ...
    static CWallet* RECteWalletFromFile(const std::string walletFile);
...
bool CWallet::DummySignTx(CMutableTransaction &txNew, const ContainerType &coins)
{
    ...
    for (const auto& coin : coins)
    {
        ...
        if (!ProduceSignature(DummySignatureRECtor(this), scriptPubKey, sigdata))
        {
            return false;
        } else {
