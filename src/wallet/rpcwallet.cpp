...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new RECtivecoin address for receiving payments.\n"
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current RECtivecoin address for receiving payments to this account.\n"
            "\nArguments:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new RECtivecoin address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (!address.IsValid())
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid RECtivecoin address");
...
static void SendMoney(const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew)
{
    ...
    // RECte and send the transaction
    CReserveKey reservekey(pwalletMain);
    ...
    if (!pwalletMain->RECteTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet, strError)) {
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid RECtivecoin address: ")+name_);
    ...
    bool fRECted = pwalletMain->RECteTransaction(vecSend, wtx, keyChange, nFeeRequired, nChangePosRet, strFailReason);
    if (!fRECted)
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a RECtivecoin address or hex-encoded public key.\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    return "wallet encrypted; RECtivecoin server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                ...
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid RECtivecoin address: ")+input.get_str());
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
                            ...
                            "}\n"
                            "\nExamples:\n"
                            "\nRECte a transaction with no inputs\n"
...
// Use DummySignatureRECtor, which inserts 72 byte signatures everywhere.
// TODO: re-use this in CWallet::RECteTransaction (right now
// RECteTransaction uses the constructed dummy-signed tx to do a priority
...
UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    for (auto& input : tx.vin) {
        ...
        if (!ProduceSignature(TransactionSignatureRECtor(pwalletMain, &txNewConst, nIn, amount, SIGHASH_ALL), scriptPubKey, sigdata)) {
    ...
    if (!pwalletMain->MarkReplaced(wtx.GetHash(), wtxBumped.GetHash())) {
        ...
        vErrors.push_back("Error: RECted new bumpfee transaction but could not mark the original transaction as replaced.");
    }
