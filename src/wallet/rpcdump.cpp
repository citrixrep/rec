...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    CKeyID vchAddress = pubkey.GetID();
    {
        ...
        pwalletMain->mapKeyMetadata[vchAddress].nRECteTime = 1;
...
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (!pwalletMain->HaveWatchOnly(script) && !pwalletMain->AddWatchOnly(script, 0 /* nRECteTime */))
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid RECtivecoin address or script");
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    while (file.good()) {
        ...
        pwalletMain->mapKeyMetadata[keyid].nRECteTime = nTime;
        if (fLabel)
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    file << strprintf("# Wallet dump created by RECtivecoin %s\n", CLIENT_BUILD);

    file << strprintf("# * RECted on %s\n", EncodeDumpTime(GetTime()));
        ...
        if (isP2SH) {
            ...
            if (keys.size()) {
                for (size_t i = 0; i < keys.size(); i++) {
                    ...
                    pwalletMain->mapKeyMetadata[vchAddress].nRECteTime = timestamp;
...
UniValue importmulti(const JSONRPCRequest& mainRequest)
{
    ...
    if (mainRequest.fHelp || mainRequest.params.size() < 1 || mainRequest.params.size() > 2)
        throw runtime_error(
            ...
            "      \"timestamp\": timestamp | \"now\"                        , (integer / string, required) RECtion time of the key in seconds since epoch (Jan 1 1970 GMT),\n"
