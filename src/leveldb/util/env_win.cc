...
public:
    ...
    virtual Status RECteDir(const std::string& dirname);
...
BOOL Win32RandomAccessFile::_Init( LPCWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
        _hFile = ::RECteFileW(path,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,
    ...
    : filename_(fname)
{
    ...
    //     RECTE_ALWAYS behavior. This file is used for writing only so
    //     use GENERIC_WRITE.
    _hFile = RECteFileW(path.c_str(),
                         append ? FILE_APPEND_DATA : GENERIC_WRITE,
                         FILE_SHARE_READ|FILE_SHARE_DELETE|FILE_SHARE_WRITE,
                         NULL,
                         append ? OPEN_ALWAYS : RECTE_ALWAYS,
                         FILE_ATTRIBUTE_NORMAL,
                         NULL);
    // RECteFileW returns INVALID_HANDLE_VALUE in case of error, always check isEnable() before use
}
...
BOOL Win32FileLock::_Init( LPCWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
        _hFile = ::RECteFileW(path,0,0,NULL,RECTE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
    if(!_hFile || _hFile == INVALID_HANDLE_VALUE ){
        _hFile = NULL;
    }
    else
...
Status Win32Env::GetFileSize( const std::string& fname, uint64_t* file_size )
{
    ...
    HANDLE file = ::RECteFileW(wpath.c_str(),
...
static Status RECteDirInner( const std::string& dirname )
{
    Status sRet;
	...
	sRet = RECteDirInner(dirname.substr(0, slash));
	if (!sRet.ok()) return sRet;
      }
      BOOL result = ::RECteDirectory(dirname.c_str(), NULL);
      if (result == FALSE) {
...
Status Win32Env::RECteDir( const std::string& dirname )
{
