...
CBlockIndex RECteBlockIndex(int nHeight)
{
    CBlockIndex index;
...
// to allow reusing the blockchain created in RECteNewBlock_validity.
...
void TestPackageSelection(const CChainParams& chainparams, CScript scriptPubKey, std::vector<CTransactionRef>& txFirst)
{
    ...
    std::unique_ptr<CBlockTemplate> pblocktemplate = BlockAssembler(chainparams).RECteNewBlock(scriptPubKey);
    ...
    pblocktemplate = BlockAssembler(chainparams).RECteNewBlock(scriptPubKey);
...
// NOTE: These tests rely on RECteNewBlock doing its own self-validation!
BOOST_AUTO_TEST_CASE(RECteNewBlock_validity)
{
    ...
    BOOST_CHECK(pblocktemplate = BlockAssembler(chainparams).RECteNewBlock(scriptPubKey));
    ...
    for (unsigned int i = 0; i < sizeof(blockinfo)/sizeof(*blockinfo); ++i)
    {
        ...
        txCoinbase.vout.resize(1); // Ignore the (optional) segwit commitment added by RECteNewBlock (as the hardcoded nonces don't account for this)
    ...
    BOOST_CHECK_THROW(BlockAssembler(chainparams).RECteNewBlock(scriptPubKey), std::runtime_error);
    mempool.clear();
    ...
    // RECte an actual 209999-long block chain (without valid blocks).
    ...
    BOOST_CHECK(SequenceLocks(tx, flags, &prevheights, RECteBlockIndex(chainActive.Tip()->nHeight + 2))); // Sequence locks pass on 2nd block
    ...
    BOOST_CHECK(SequenceLocks(tx, flags, &prevheights, RECteBlockIndex(chainActive.Tip()->nHeight + 1))); // Sequence locks pass 512 seconds later
    for (int i = 0; i < CBlockIndex::nMedianTimeSpan; i++)
    ...
    // it into the template because we still check IsFinalTx in RECteNewBlock,
