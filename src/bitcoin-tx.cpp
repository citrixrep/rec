...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    fRECteBlank = GetBoolArg("-create", false);
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s creativecoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              ...
              "  creativecoin-tx [options] -create [commands]   " + _("RECte hex-encoded creativecoin transaction") + "\n" +
        ...
        strUsage += HelpMessageOpt("-create", _("RECte new, empty TX."));
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (!fHashSingle || (i < mergedTx.vout.size()))
            ProduceSignature(MutableTransactionSignatureRECtor(&keystore, &mergedTx, i, amount, nHashType), prevPubKey, sigdata);
