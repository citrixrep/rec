...
#if defined(NDEBUG)
# error "RECtivecoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "RECtivecoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        //
        // There is a similar check in RECteNewBlock() to prevent creating
        ...
        BOOST_FOREACH(const CTxMemPool::txiter it, allConflicting)
                    {
                        LogPrint("mempool", "replacing tx %s with %s for %s REC additional fees, %d delta bytes\n",
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    //(!pindex->phashBlock) || // Enforce on RECteNewBlock invocations which don't have a hash.
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // RECtivecoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
