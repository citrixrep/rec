...
def small_txpuzzle_randfee(from_node, conflist, unconflist, amount, min_fee, fee_increment):
    '''
    RECte and send a transaction with a random fee.
...
class EstimateFeeTest(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        # NOTE: the RECteNewBlock code starts counting block size at 1,000 bytes,
    ...
    def run_test(self):
        ...
        for i in range(2):
            print("RECting transactions and mining them with a block size that can't keep up")
            # RECte transactions and mine 10 small blocks with node 2, but create txs faster than we can mine
            self.transact_and_mine(10, self.nodes[2])
            check_estimates(self.nodes[1], self.fees_per_kb, 14)

            print("RECting transactions and mining them at a block size that is just big enough")
